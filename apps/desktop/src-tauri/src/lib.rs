...
const AUTH_DEBUG_LOG_PATH: &str = "/private/tmp/lexelo-auth.log";
...
fn auth_log(op: &str, step: &str) {
    let line = format!("[AUTH][{}][{}] {}", log_now_ms(), op, step);
    eprintln!("{line}");
    append_auth_log_line(&line);
}
...
fn append_auth_log_line(line: &str) {
    if let Ok(mut file) = fs::OpenOptions::new()
        .create(true)
        .append(true)
        .open(AUTH_DEBUG_LOG_PATH)
    {
...
fn is_keychain_item_already_exists_message(message: &str) -> bool {
    message.to_lowercase().contains("already exists in the keychain")
}
...
fn create_sign_in_entity_errors(entity: &CreateSignInError) -> Option<&clerk_fapi_rs::models::ClerkErrors> {
    match entity {
...
fn extract_clerk_create_sign_in_404_code(error: &ClerkApiError<CreateSignInError>) -> Option<String> {
    match error {
...
fn desktop_callback_url(flow: DesktopAuthFlow, state: &str, provider: Option<&str>) -> String {
    let flow_value = match flow {
        DesktopAuthFlow::V1 => "v1",
        DesktopAuthFlow::V2 => "v2",
    };
    let mut query = format!("state={}&flow={flow_value}", urlencoding::encode(state));
    if let Some(provider) = provider {
        query.push_str("&provider=");
        query.push_str(&urlencoding::encode(provider));
    }
...
fn set_cached_stronghold_password(app: &tauri::AppHandle, password: Option<String>) -> Result<(), String> {
    let runtime = app.state::<SecureStorageRuntime>();
...
fn initialize_stronghold_with_recovery_internal(
    ...
    auth_log("initialize_stronghold_with_recovery", "snapshot_path_resolved");
    let password = get_or_create_stronghold_password(app)?;
    let key = stronghold_key_from_password_string(&password);
    auth_log("initialize_stronghold_with_recovery", "stronghold_new_start");
...
async fn load_secure_auth_state_without_timeout(
    ...
    if !stronghold_open_started {
        return match load_task.await {
            Ok(Ok(state)) => {
                auth_log_elapsed(op, "load_secure_auth_state_without_timeout_done", started_at);
                Ok(state)
            }
    ...
    match timeout(Duration::from_millis(STRONGHOLD_BOOTSTRAP_TIMEOUT_MS), load_task).await {
        Ok(Ok(Ok(state))) => {
            auth_log_elapsed(op, "load_secure_auth_state_without_timeout_done", started_at);
            Ok(state)
        }
...
async fn save_secure_auth_state_without_timeout(
    ...
    let started_at = Instant::now();
    match save_secure_auth_state(app, &state) {
        Ok(()) => {
            auth_log_elapsed(op, "save_secure_auth_state_without_timeout_done", started_at);
            Ok(())
        }
        Err(error) => {
...
async fn clear_secure_auth_state_with_timeout(app: &tauri::AppHandle) -> Result<(), String> {
    let started_at = Instant::now();
    let app_handle = app.clone();
    let clear_task = tokio::task::spawn_blocking(move || clear_secure_auth_state(&app_handle));

    match timeout(Duration::from_millis(STRONGHOLD_OP_TIMEOUT_MS), clear_task).await {
        Ok(Ok(result)) => {
            auth_log_elapsed("auth_sign_out", "clear_secure_auth_state_with_timeout_done", started_at);
            result
        }
        Ok(Err(error)) => Err(format!("Stronghold clear task join failed: {error}")),
        Err(_) => Err("Stronghold clear timed out".to_string()),
    }
}
...
async fn ensure_clerk_ready(
    ...
    let hydrated_state: Option<SecureAuthState> = if should_hydrate && !has_runtime_session {
        ...
        if let Some(stored) = loaded.as_ref() {
            app.clerk().set_client_authorization_header(stored.authorization_header.clone());
            auth_log("ensure_clerk_ready", "ensure_clerk_initialized_after_hydration_start");
            if let Err(error) = app.ensure_clerk_initialized().await {
                if let Ok(mut guard) = hydration.hydrated.lock() {
                    *guard = false;
                }
            ...
            auth_log("ensure_clerk_ready", "ensure_clerk_initialized_after_hydration_done");
        }
        loaded
    } else {
        None
...
async fn wait_for_active_session(
    ...
    for _ in 0..10 {
        ...
        if session_id.as_deref() == Some(expected_session_id) {
            auth_log_elapsed("wait_for_active_session", "active_session_found", started_at);
            return Ok(());
        }
...
async fn persist_runtime_auth_state_after_active(
    ...
    .await;
    auth_log_elapsed("persist_runtime_auth_state_after_active", "done", started_at);
    result
}
...
fn validate_web_sign_in_url(sign_in_url: &str) -> Result<Url, String> {
    ...
    if !parsed.path().starts_with(&expected_sign_in_prefix) {
        return Err("Sign-in URL path must start with '/sign-in' under WEB_AUTH_BASE_URL".to_string());
    }
...
fn flow_from_url(url: &Url) -> Option<DesktopAuthFlow> {
    let normalized_path = url.path().trim_end_matches('/');
    let host_path = format!("{}{}", url.host_str().unwrap_or_default(), normalized_path);

    if host_path == "auth/v1" || url.path() == "/auth/v1" {
        return Some(DesktopAuthFlow::V1);
    }

    if host_path == "auth/v2" || url.path() == "/auth/v2" {
        return Some(DesktopAuthFlow::V2);
    }

    if host_path == "auth" || url.path() == "/auth" || url.host_str() == Some("auth") {
        if let Some(explicit_flow) = url.query_pairs().find_map(|(key, value)| {
            if key == "flow" {
            ...
            return match explicit_flow.as_str() {
                "v1" => Some(DesktopAuthFlow::V1),
                "v2" => Some(DesktopAuthFlow::V2),
                _ => None,
            };
        }

        if url.query_pairs().any(|(key, _)| key == "provider") {
            return Some(DesktopAuthFlow::V1);
        }
        return Some(DesktopAuthFlow::V2);
    }
...
fn parse_desktop_auth_callback_url(raw_url: &str) -> Option<DesktopAuthCallbackPayload> {
    ...
    Some(DesktopAuthCallbackPayload {
        flow,
        ticket,
        state,
    })
...
fn validate_pending_oauth(
    ...
    } else if request.flow == DesktopAuthFlow::V1 {
        return Err("Invalid OAuth callback state.".to_string());
    }
            ...
            if let Some(created_session_id) = sign_in.created_session_id.clone() {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    if set_active_result.is_ok() {
                        ...
                        let _ = persist_runtime_auth_state_after_active(
                            &app_handle,
                            &created_session_id,
                        )
                        .await;
                ...
                auth_log("map_sign_in_response", "complete_without_created_session_id");
                let app_handle = app.clone();
                tauri::async_runtime::spawn(async move {
                    let async_started = Instant::now();
                    let runtime_payload = auth_status_payload_from_runtime(&app_handle);
                    emit_auth_state(&app_handle, "map_sign_in_response_async", &runtime_payload);
                    let _ = persist_runtime_auth_state(&app_handle).await;
                    auth_log_elapsed("map_sign_in_response_async", "persist_without_session_done", async_started);
                });
            }
            {
...
async fn auth_bootstrap(
    ...
    let hydrated_state = match ensure_clerk_ready(&app, &hydration, true).await {
        Ok(state) => state,
        Err(error) => {
            auth_log("auth_bootstrap", &format!("ensure_clerk_ready_error={error}"));
            if is_keychain_access_required_error(&error) {
                emit_auth_phase(&app, "auth_bootstrap", AUTH_PHASE_WAITING_FOR_SECURE_STORAGE);
                auth_log_elapsed(
                    "auth_bootstrap",
            ...
            auth_log_elapsed("auth_bootstrap", "ensure_clerk_ready_failed_using_stored", started_at);
            let fallback =
                auth_status_payload_from_stored(load_secure_auth_state_with_timeout(&app, "auth_bootstrap").await);
...
async fn auth_status(
    ...
    let display_name = runtime_user
        .as_ref()
        .and_then(derive_user_display_name);
...
async fn auth_sign_in_password(
    ...
    if let Some(existing) = complete_if_runtime_session_exists(&app, "auth_sign_in_password").await? {
    ...
    let sign_in = match sign_in {
        ...
        Err(error) => {
            ...
            {
                auth_log_elapsed("auth_sign_in_password", "done_error_recovered_from_session", started_at);
                return Ok(existing);
            }
...
async fn auth_prepare_second_factor(
    ...
    let started_at = Instant::now();
    auth_log("auth_prepare_second_factor", &format!("start strategy={strategy}"));
    let _ = ensure_clerk_ready(&app, &hydration, false).await?;
    let sign_in_id = {
        let lock = pending
            .sign_in_id
    ...
    if strategy == "email_code" {
        auth_log("auth_prepare_second_factor", "prepare_sign_in_factor_two_start");
        let sign_in = app
            .clerk()
            .get_fapi_client()
        ...
        auth_log("auth_prepare_second_factor", "prepare_sign_in_factor_two_done");
        let result = map_sign_in_response(&app, &pending, sign_in).await;
...
async fn auth_attempt_second_factor(
    ...
    let started_at = Instant::now();
    auth_log("auth_attempt_second_factor", &format!("start strategy={strategy}"));
    let _ = ensure_clerk_ready(&app, &hydration, false).await?;
    let sign_in_id = {
        let lock = pending
            .sign_in_id
    ...
    auth_log("auth_attempt_second_factor", "attempt_sign_in_factor_two_start");
    let sign_in = app
        .clerk()
        .get_fapi_client()
    ...
    auth_log("auth_attempt_second_factor", "attempt_sign_in_factor_two_done");
...
async fn auth_start_oauth(
    ...
    let started_at = Instant::now();
    auth_log("auth_start_oauth", &format!("start provider={provider}"));
    let _ = ensure_clerk_ready(&app, &hydration, false).await?;
    let strategy = match provider.as_str() {
        "google" => "oauth_google",
        "microsoft" => "oauth_microsoft",
        _ => return Err(format!("Unsupported provider: {provider}")),
    };
    let state = create_auth_state();
    let redirect_url = desktop_callback_url(DesktopAuthFlow::V1, &state, Some(&provider));
    set_pending_oauth_request(&pending_oauth, DesktopAuthFlow::V1, state)?;

    auth_log("auth_start_oauth", "create_sign_in_start");
    let sign_in = app
        .clerk()
        .get_fapi_client()
        .create_sign_in(
        ...
        .map_err(|e| {
            ...
            format!("OAuth start failed: {e}")
        })?;
    auth_log("auth_start_oauth", "create_sign_in_done");

    let result = map_sign_in_response(&app, &pending, sign_in).await;
    auth_log_elapsed("auth_start_oauth", "done", started_at);
    result
}
...
async fn auth_start_web_sign_in(
    ...
    let state = create_auth_state();
    let callback_url = desktop_callback_url(DesktopAuthFlow::V2, &state, None);
    set_pending_oauth_request(&pending_oauth, DesktopAuthFlow::V2, state)?;

    let mut auth_url: String = validate_web_sign_in_url(&sign_in_url)?.into();
...
async fn auth_handle_callback_url(
    ...
    let Some(callback) = parse_desktop_auth_callback_url(&url) else {
        auth_log_elapsed("auth_handle_callback_url", "ignored_non_auth_url", started_at);
        return Ok(None);
    };

    validate_pending_oauth(&pending_oauth, &callback)?;
    let _ = ensure_clerk_ready(&app, &hydration, false).await?;
    if let Some(existing) = complete_if_runtime_session_exists(&app, "auth_handle_callback_url").await? {
        auth_log_elapsed("auth_handle_callback_url", "done_existing_session", started_at);
    ...
    let sign_in = match sign_in {
        ...
        Err(error) => {
            ...
            {
                auth_log_elapsed("auth_handle_callback_url", "done_error_recovered_from_session", started_at);
                return Ok(Some(existing));
            }
...
async fn auth_sign_out(
    ...
    let app_for_remote_sign_out = app.clone();
    tauri::async_runtime::spawn(async move {
        if app_for_remote_sign_out.ensure_clerk_initialized().await.is_ok() {
            let _ = app_for_remote_sign_out.clerk().sign_out(None).await;
        }
    });

    app.clerk().set_client_authorization_header(None);
    if let Err(error) = clear_secure_auth_state_with_timeout(&app).await {
