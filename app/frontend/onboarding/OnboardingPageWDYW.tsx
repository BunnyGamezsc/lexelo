...
import {ArrowLeft, ArrowRight, BookOpen, GraduationCap, CheckCircle, Globe, Crown} from "lucide-react";
...
const OnboardingPageWDYW = ({ setGoBack, goNext, goBack }) => {
    ...
    const handleOptionToggle = (option) => {
        ...
        if ((option === 'guided' || option === 'scholar') && selectedLanguages.english && !selectedLanguages.latin) {
            return;
        }
    ...
    const handleLanguageToggle = (language) => {
        setSelectedLanguages(prev => {
            ...
            if (language === 'latin' && prev.latin && newLanguages.english && !newLanguages.latin) {
    ...
    const isLearningStyleDisabled = selectedLanguages.english && !selectedLanguages.latin;

    const canContinue = (selectedLanguages.latin && selectedOptions.scholar) || (!selectedLanguages.latin && selectedLanguages.english) || (selectedLanguages.latin && selectedOptions.guided)
    return (
            ...
            <div className="max-w-4xl mx-auto mb-8 animate-fadeInUp animation-delay-500">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-xl font-semibold text-(--lingua-grey) text-left">Learning Style</h3>
                        ...
                        <div 
                            ...
                            onClick={() => !isLearningStyleDisabled && handleOptionToggle('guided')}
                        >
                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                                ...
                                {selectedOptions.guided && !isLearningStyleDisabled && <CheckCircle className="w-4 h-4 text-white" />}
                            </div>
                            <BookOpen className={`w-6 h-6 flex-shrink-0 ${isLearningStyleDisabled ? 'text-gray-400' : 'text-(--lingua-grey)'}`} />
                            <div className="text-left">
                                <div className={`font-semibold ${isLearningStyleDisabled ? 'text-gray-400' : 'text-(--lingua-grey)'}`}>
                                    Guided Learning
                                    {isLearningStyleDisabled && <span className="text-xs ml-2">(Latin only)</span>}
                                </div>
                                <div className={`text-sm ${isLearningStyleDisabled ? 'text-gray-400' : 'text-[#5A5A5A]'}`}>
                        ...
                        <div 
                            ...
                            onClick={() => !isLearningStyleDisabled && handleOptionToggle('scholar')}
                        >
                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                                isLearningStyleDisabled 
                                    ? 'border-gray-300 bg-gray-100'
                                ...
                                {selectedOptions.scholar && !isLearningStyleDisabled && <CheckCircle className="w-4 h-4 text-white" />}
                            </div>
                            <Crown className={`w-6 h-6 flex-shrink-0 ${isLearningStyleDisabled ? 'text-gray-400' : 'text-(--lingua-grey)'}`} />
                            <div className="text-left">
                                <div className={`font-semibold ${isLearningStyleDisabled ? 'text-gray-400' : 'text-(--lingua-grey)'}`}>
                                    Already a Scholar
                                    {isLearningStyleDisabled && <span className="text-xs ml-2">(Latin only)</span>}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            ...
                            >
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                                    selectedLanguages.latin ? 'border-(--lingua-grey) bg-(--lingua-grey)' : 'border-gray-300'
                                }`}>
                                    {selectedLanguages.latin && <CheckCircle className="w-4 h-4 text-white" />}
                                </div>
                                <div className="text-left">
                                    <div className="font-semibold text-(--lingua-grey)">Latin</div>
                                    <div className="text-sm text-[#5A5A5A]">Classical language learning</div>
                                </div>
                            </div>
                            ...
                            >
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                                    selectedLanguages.english ? 'border-(--lingua-grey) bg-(--lingua-grey)' : 'border-gray-300'
                                }`}>
                                    {selectedLanguages.english && <CheckCircle className="w-4 h-4 text-white" />}
                                </div>
                                <div className="text-left">
                                    <div className="font-semibold text-(--lingua-grey)">Other Flashcards</div>
                                    <div className="text-sm text-[#5A5A5A]">Custom study sets and vocabulary</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {selectedLanguages.latin && selectedOptions.guided && (
                    <div className="mt-8 space-y-4 animate-fadeInUp">
                        <h3 className="text-xl font-semibold text-(--lingua-grey) text-center">What's your Latin level?</h3>
                        <div className="max-w-md mx-auto">
                            ...
                            >
                                ...
                                <option value="elementary">Elementary - I know basic words</option>
                                <option value="intermediate">Intermediate - I can read simple texts</option>
                                <option value="advanced">Advanced - I'm comfortable with grammar</option>
                            </select>
                        </div>
                    </div>
                )}
